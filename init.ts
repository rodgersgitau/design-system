...
async function getNames() {
	...
	const templates = sort(await globby("./atomic-design/templates/*/package.json")).map(item =>
		getLastDir(item)
	);
	const utils = sort(await globby("./utils/*/package.json")).map(item => getLastDir(item));
	return { atoms, ions, molecules, organisms, templates, utils };
}
